...
bool PluginManager::load(QWidget* widget, const QString& name) {
    LOG_DEBUG_LEVEL_PREFIX("Loading plug-in (" + name.toStdString() + ").", "PluginManager");

    const auto& itr = mPlugins.find(name);
    if(itr != mPlugins.end()) {
        ...
        if(plugin) {
            ...
            if(plugin->startUp(widget)) {
                ...
                LOG_DEBUG_LEVEL_PREFIX("Successfully loading the plug-in.", "PluginManager");
                return true;
            }
        }
        else {
...
void PluginManager::unload() {
    if(mCurrentPlugin.mPlugin) {
        LOG_DEBUG_LEVEL_PREFIX(
            "Unloading plug-in (" + mCurrentPlugin.mPlugin->getMetaData().mName.toStdString() + ").", "PluginManager");
...
bool PluginManager::scan(const QString& path) {
    LOG_DEBUG_LEVEL_PREFIX("Scanning for plug-ins in " + path.toStdString(), "PluginManager");
    ...
    foreach(QFileInfo fileInfo, pPath.entryInfoList(QDir::Files | QDir::NoDotAndDotDot)) {
        ...
        LOG_DEBUG_LEVEL_PREFIX("Checking current file: " + fileInfo.fileName().toStdString(), "PluginManager");
        if(QLibrary::isLibrary(fileName)) {
            ...
            if(!meta.mName.isEmpty()) {
                ...
                LOG_DEBUG_LEVEL_PREFIX("Adding plug-in into the list.", "PluginManager");
            }
            else {
                LOG_WARNING_LEVEL_PREFIX("Plug-in missing name description.", "PluginManager");
                delete loader;
            }
        }
        else {
            LOG_DEBUG_LEVEL_PREFIX("Skipping due to not valid plug-in file.", "PluginManager");
        }
    }
