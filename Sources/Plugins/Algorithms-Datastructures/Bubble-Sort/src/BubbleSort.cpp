...
void BubbleSort::generateData(ViewDataSet& data, QGridLayout* layout, qint32 nums) {
...
void BubbleSort::generateRealtimeData() {
    ...
    if(!mRealtimeVisualised->isChecked() || mNumsRealtimeData->value() > 300) {
        ...
        generateData(mRealtimeDataSet, mRealtimeLayout, mNumsRealtimeData->value());
    }
    ...
    if(!mIterator.finished) {
        ...
        if(mIterator.i > mIterator.ei) {
            ...
            if(mIterator.j < mIterator.i) {
                ...
                if(mIterator.state == HL_COMPARE_CONTINUE || mIterator.state == HL_SWITCH_CONTINUE
                   || mIterator.state == HL_SWITCH) {
                    ...
                    if(swapData) {
                        if(mIterator.state != HL_SWITCH && mIterator.state != HL_SWITCH_CONTINUE) {
                            ...
                            switchData(itemA, mIterator.j, itemB, mIterator.j + 1, mVisualLayout);
...
bool BubbleSort::startUp(QWidget* parent) {
    ...
    // Connections
    // ----------------------------------------
    connect(mViewGenerate, &QPushButton::clicked, this, &BubbleSort::generateViewData);
    connect(mViewController->mButtonForward, &QPushButton::clicked, this, &BubbleSort::sortVisual);
    connect(mViewController->mTimer, &QTimer::timeout, this, &BubbleSort::sortVisual);
    connect(mTabWidget, &QTabWidget::currentChanged, this, &BubbleSort::tabChanged);
    connect(mRealtimeGenerate, &QPushButton::clicked, this, &BubbleSort::generateRealtimeData);
    connect(mRealtimeSort, &QPushButton::clicked, this, &BubbleSort::sortRealtime);

    return true;
}


void BubbleSort::switchData(const int idxA, const int idxB, QGridLayout* layout) {
    switchData(getItem(idxA, layout), idxA, getItem(idxB, layout), idxB, layout);
}


void BubbleSort::switchData(QLabel* itemA, const int idxA, QLabel* itemB, const int idxB, QGridLayout* layout) {
    // Retrieve indices.
